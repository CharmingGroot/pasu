...
- **② 도구 호출 가드 (협조적·인프로세스)** (`pasu-rig`) — 선언된 도구 호출을 검사하고, 필요하면 사람이 승인(HITL)하며, LLM egress도 정책으로 거릅니다. 맥락은 풍부하지만 이 계층만으로는 우회될 수 있습니다.
...
> 솔직히 말하면: pasu는 아직 MVP입니다. 보안 인증도, 프로덕션 도입 사례도 없습니다. 이 니치를 실제로 구현한 '동작하는 셀프호스팅 레퍼런스'이지, 곧바로 들여놓는 인증 어플라이언스가 아닙니다.

## 비교

전 세계에서 제일 낫다는 주장이 아니라, 대안들이 더 무겁거나 아예 돌지 못하는 **온프렘/규제** 환경에서의 적합성입니다.
...
| 에이전트 의도 맥락 (도구 호출·HITL) | ✅ | ✅ | ❌ | ✅ |
| 감사 로그 (JSONL / OTLP, 내 스택) | ✅ | 일부 | ~ | ✅ (공급사 클라우드) |
| 언어/프레임워크 무관 | ✅ | ❌ | ✅ | ~ |
...
다음 계획: 정밀 DNS 응답 스니핑(toFQDN — 커널에서 접미 호스트까지 처리), eBPF 계층 감사 emit, 컨트롤 플레인 API와 더 나은 UI, 그리고 crates.io 릴리스(현재 rig는 git 고정).
