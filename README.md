...
> **에이전트를 믿지 않고도 egress를 통제합니다 — 우리 네트워크 안에서.**
> 인프로세스 훅은 에이전트가 *선언한* 호출만 볼 수 있어서, 도구가 자기 소켓을 직접 열면 그대로 빠져나갑니다.
> pasu는 그 위에 **에이전트가 우회할 수 없는 커널 eBPF 가드**를 두고, 모든 판정을 감사 로그로 남깁니다.
> 전부 호스트 안에서 돌아가며, SaaS로 보내는 데이터는 없습니다. **enforcing > cooperative.**

---

## pasu가 필요한 이유

AI 에이전트는 프롬프트 인젝션(prompt injection)에 취약하고, 한번 장악당한 에이전트는 데이터를 서슴없이 빼돌립니다. 온프렘·망분리·규제 환경에서 에이전트를 돌린다면 두 가지가 분명해집니다. 에이전트 트래픽을 클라우드/SaaS 가드로 보낼 수 없다는 것, 그리고 쉽게 우회당하는 단일 검사가 아니라 다층 방어와 감사 증적이 필요하다는 것.
...
흔치 않은 건 이 *조합*을 셀프호스티드 서버 한 대에서 제공한다는 점입니다.
...
- **쿠버네티스도 클라우드도 필요 없습니다.** Linux 호스트 하나면 되고, `pasu run`으로 어떤 에이전트든 감쌉니다. K8s 네이티브 네트워크 정책 엔진은 강력하지만 온프렘 서버 한 대에는 과하고, SaaS 에이전트 가드는 망분리망 안에서는 아예 동작하지 못합니다.
- **망분리 환경에서 동작합니다.** 런타임에 외부로 연결하지 않습니다. 텔레메트리 전송은 선택이며, 그 대상도 여러분의 collector입니다.
- **커널 인라인 egress + 에이전트 의도 + 감사**를 한꺼번에 제공합니다. 대부분의 도구는 이 셋 중 하나만 다룹니다.
- **Apache-2.0**, 감사 가능한 Rust로 작성했고, 모든 crate는 trait 뒤에 있어 교체할 수 있습니다.

## 한계 (솔직하게)

pasu가 **하지 않는** 것들 — 도입 전에 알아야 합니다.

- **커널 강제는 Linux 전용.** macOS·Windows에서는 협조적 계층(LLM-API 프록시)만 돌고, egress 강제는 없습니다.
- **eBPF에는 권한이 필요합니다**(root / CAP_BPF). 프록시 계층은 무권한으로 돌지만, 강제 계층은 커널 권한이 필요합니다.
- **프록시 계층은 단독으로는 우회 가능합니다** — 그래서 커널 계층이 있습니다. 도구가 자체 소켓으로 나가면 프록시는 못 보고, 커널이 최종 차단합니다.
- **egress 판정은 L3/L4(IP·도메인) 수준입니다.** TLS 페이로드나 L7 콘텐츠 검사(DLP)는 하지 않습니다 — 허용된 도메인으로의 데이터 유출(EchoLeak류)은 allowlist로 막지 못합니다.
- **스트리밍(SSE) 응답의 도구 호출은 아직 검사하지 않습니다**(그대로 통과). 논스트리밍만 지원하며, DNS 인식은 best-effort입니다(정밀 toFQDN은 예정).
- **프롬프트 인젝션·모델 오작동 자체는 막지 않습니다.** pasu는 egress와 도구 의도를 통제할 뿐 모델 행동을 교정하지 않습니다 — 입력 방어가 아니라 최종 방어선입니다.
- **아직 MVP입니다.** 보안 인증도, 프로덕션 도입 사례도 없습니다 — 이 니치의 '동작하는 셀프호스팅 레퍼런스'이지 곧바로 들여놓는 인증 어플라이언스가 아닙니다.
...
IPv4 allow는 정적 항목이 되고, 정확한 호스트명은 주기적으로 재해석되며, 접미 패턴(`.openai.com`)은 리포트만 됩니다 — DNS 응답 스니핑이 들어오기 전까지는 훅 계층에서만 적용됩니다. 커널은 기본 차단이라, 변환 결과는 정책보다 좁아질 뿐 넓어지지 않습니다.
